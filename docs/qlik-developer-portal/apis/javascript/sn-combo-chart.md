---
source: https://qlik.dev/apis/javascript/sn-combo-chart/
last_updated: 2026-09-11T16:13:57Z
---

# Nebula Combo chart

`Version: 1.50.0` | _stable_
...
- [MeasureColor](#measurecolor-object)
...
- [SeriesMarkerOptions](#seriesmarkeroptions-object)
...
| --- | --- | --- | --- | --- |
| `barGrouping` | object | Yes | - | Bar grouping. |
...
| `measureAxes` | [measureAxis](#measureaxis-object)[] | Yes | - | Measure axes settings. Must be an array with 2 measureAxis objects. |
| `nullMode` | 'gap' \| 'connect' \| 'zero' | No | "gap" | Null value presentation |
| `orientation` | 'vertical' \| 'horizontal' | Yes | "vertical" | Chart orientation setting. If vertical, the dimension axis can only be docked on bottom or top and measure axis on left or right. If horizontal, the dimension axis can only be docked on left or right and measure axis on bottom or top. |
...
| `scrollStartPos` | number | Yes | 0 | Scroll Alignment. Decides if the scroll starts at the beginning or end of the data. Where 0 means at the  beginning and 1 at the end. |
...
<details>
<summary>Properties of `barGrouping`</summary>

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `grouping` | 'stacked' \| 'grouped' | Yes | "grouped" | Group setting |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `show` | boolean | Yes | false | Show line series data points. |
| `showBarLabels` | boolean | Yes | false | Show labels on bar series. |
| `showLineLabels` | boolean | Yes | false | Show labels on line series data points. |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `axisDisplayMode` | 'auto' \| 'custom' \| 'max' | Yes | "auto" | Set to specify axis display mode (number of bars). |
...
| `refLines` | [refLine](#refline-object)[] | Yes | - | Array of primary measure axis reference line definitions |
| `refLines2` | [refLine](#refline-object)[] | Yes | - | Array of secondary measure axis reference line definitions |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `bar` | [ChartStyling](#chartstyling-object) | Yes | - |  |
| `key` | string | Yes | - | This should be set to `"bar"` (Determines which component the settings apply to). |
| `style` | [BarStyling](#barstyling-object) | Yes | - |  |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `strokeColor` | [paletteColor](#palettecolor-object) | Yes | - |  |
| `strokeWidth` | 'none' \| 'small' \| 'medium' \| 'large' | Yes | "none" | Set to specify stroke width. |
| `width` | number | Yes | - | A number greater than 0 and less than or equal to 1. |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `label` | [LabelValueStyling](#labelvaluestyling-object) | Yes | - | Bar labels if bar and data point labels if line. |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `id` | string | Yes | - | One of: `colorByAlternative`: colors the chart using different dimensions (can be used together with color.mode="byDimension"), `colorByExpression` together with color.mode="byExpression" or `colorByMeasureExpression` togehter with color.mode="byExpression" on the current measure. |
...
| `cId` | string | No | - | ID used by the Qlik Sense. Must be unique within the current chart. |
| `color` | [MeasureColor](#measurecolor-object) | No | - | Set to define color for this measure |
| `modifiers` | [Modifier](#modifier-alias)[] | No | - | Set to apply a modifier to a measure. |
| `series` | [Series](#series-object) | No | - | Set to define type of series and series options |
...
| `line` | [ChartStyling](#chartstyling-object) | Yes | - |  |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `max` | number \| ValueExpression | Yes | - | Axis max value. `"autoMinMax"` must be set to false and `"minMax"` must be set to `"max"` or `"minMax"` to use this property |
| `min` | 'min' \| 'max' \| 'minMax' | Yes | - | Axis min value. `"autoMinMax"` must be set to false and `"minMax"` must be set to `"min"` or `"minMax"` to use this property |
| `minMax` | 'min' \| 'max' \| 'minMax' | Yes | - | Set custom max/min |
| `autoMinMax` | boolean | No | true | Automatic max/min |
| `dock` | 'near' \| 'far' | No | "near" | Axis docking position |
| `show` | 'all' \| 'labels' \| 'title' \| 'none' | No | "all" | Labels and title |
| `spacing` | number | No | 1 | Axis scale |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `mode` | 'primary' \| 'byExpression' | Yes | "primary" | Sets the coloring mode for the measure when coloring has been switched on Can be one of: * `primary`: a single color (by default blue) is used. single color is the default setting. * `byExpression`: coloring is based on an expression. The expression are set in the `"colorExpression"` property. |
| `on` | boolean | Yes | false | Set if the color settings should be used |
| `paletteColor` | [paletteColor](#palettecolor-object) | Yes | "{" | index: 6 } The paletteColor object is used to define the color when you color by single color `("mode": "primary")`. |
| `colorExpression` | string | No | - | Sets the color expression to be used when `"mode": "byExpression"` is defined. Expression is a color code. Supported formats are: `RGB`, `ARGB` and `HSL` |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `options` | [SeriesMarkerOptions](#seriesmarkeroptions-object) | Yes | - | Set options specific for a series. |
| `axis` | 0 \| 1 | No | 0 | Set which axes the series should be represented on. 0 for primary axis and 1 for secondary axis. Defaults to primary with less than 2 measure, otherwise secondary axis. |
| `type` | 'bar' \| 'line' \| 'marker' | No | "bar" | Set the type of series. Defaults to 'bar' with less than 2 measure, otherwise 'line'. |
...
| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `fill` | boolean | No | true | Set to true if the marker should be filled with a color. If false, only stroke of the marker is rendered. |
| `type` | 'circle' \| 'line' \| 'diamond' \| 'triangle' \| 'rect' | No | "circle" | Set the type of marker. |
