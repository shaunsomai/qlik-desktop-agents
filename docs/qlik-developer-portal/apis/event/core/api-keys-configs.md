---
source: https://qlik.dev/apis/event/core/api-keys-configs/
...
- [com.qlik.core.api-keys-config.updated](#comqlikcoreapi-keys-configupdated)
...
| `type` | `string` | Yes | Identifies the type of event related to the originating occurrence. Metadata: default = "com.qlik.core.api-keys-config.updated". |
...
| `data` | `object` | No | Details of the updated API key configuration. |
...
| `maxKeysPerUser` | `integer` | Yes | Maximum number of active API keys per user. |
| `maxApiKeyExpiry` | `string` | Yes | Maximum allowed lifetime of an API key, in ISO 8601 duration format. |
