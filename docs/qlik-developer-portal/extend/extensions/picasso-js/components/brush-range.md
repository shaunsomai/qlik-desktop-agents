---
source: https://qlik.dev/extend/extensions/picasso-js/components/brush-range/
last_updated: 2026-06-02T18:15:45+01:00
---

# Brush - Range

Brush range component enables [brushing](https://qlik.dev/extend/extensions/picasso-js/components/main-concepts/brushing) a range, or set of ranges, of
continuous data.
...
No [data](https://qlik.dev/extend/extensions/picasso-js/components/main-concepts/data) is required as input directly, instead the data is implicitly
fetched from the referenced [scale](https://qlik.dev/extend/extensions/picasso-js/components/main-concepts/scales). The scale must however be [linear](https://qlik.dev/extend/extensions/picasso-js/components/main-concepts/scales).

## Layout

There are two ways to set the layout, either by referencing a `target` components,
in which case, the rendering area extends from the `target` components to the
start of the opposite docking area. Ex. if `target` is docked to the `right`,
then the rendering area includes the `right` area and the `center` area.
The other option, which is default, is if no `target` component is configured,
then the `center` area is used.
...
For more information, see the [API reference](https://qlik.dev/apis/javascript/picasso-js/#definitions-componentbrushrange).
