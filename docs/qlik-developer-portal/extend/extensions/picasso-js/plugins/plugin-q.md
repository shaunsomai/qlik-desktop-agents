---
source: https://qlik.dev/extend/extensions/picasso-js/plugins/plugin-q/
last_updated: 2026-06-02T18:15:45+01:00
---

# q

The `q` plugin registers a `q` dataset type that makes it a bit easier to extract
...
This dataset type understands the QIX hypercube format and its internals, making
it a bit easier to traverse and extract values from an otherwise complex structure.
...
Dimensions, measures, attribute expressions and attribute dimensions are all
recognized as fields and can be found using either the path or the title of the field:
...
Assuming you have a hypercube containing dimensions *Year* and *Month*, a measure
*Sales* and an attribute expression on the first dimension containing color values:
...
The default `value` accessor for a field depends on the field type and the
`qMode` property of the hypercube:

- For measures and attribute expressions: `cell => cell.qNum` or `cell => cell.qValue`
- For dimensions and attribute dimensions: `cell => cell.qElemNumber` or
  `cell => cell.qElemNo`
...
The QIX selections helper provides a mapping from brushed data points to
suitable QIX selections.

### By dimension value

Brushing dimension values is done by adding the value of `qElemNumber` to the
...
Calling `picassoQ.selections` with this instance generates relevant QIX
...
Here rows `10` and `13` have been brushed on dimension `1`, and
rows `11` and `17` on dimension `0`. To extract the relevant information,
`byCells` is enabled:
...
`qDimensionInfo/1`, in this case. To use values from another dimension,
`primarySource` should be set:
...
Therefore, to calculate the index, `layout` containing the hypercube needs to be
...
- total number of attribute expressions in measures preceding the one
  specified: `2` (from first measure)
...
The `q` plugin comes bundled with a [formatter](https://qlik.dev/extend/extensions/picasso-js/plugins/main-concepts/formatting)
attached the to data `field`. To use it, no configuration other then using the
[q-dataset](#q-dataset) is required.

[localeinfo]: https://qlik.dev/apis/json-rpc/qix/schemas#%23%2Fdefinitions%2Fschemas%2Fentries%2FLocaleInfo
